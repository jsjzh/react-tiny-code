...
const createAPI = (host: string, apiConfig: IRequestConfig = {}) => {
  ...
  function request<T>(endpoint: string, reqConfig: IRequestConfig = {}): IRequestResult<T> {
    const config = { ...apiConfig, ...reqConfig };
    const { handleOption, handleResp = (resp: any) => resp.data, ...reqOpts } = config;
    ...
    function checkResp(data: { success: boolean; data: T; msg: string; code: number }) {
      if (data.success) {
        return data;
      }
   ...
   * @param {IRequestConfig} config
   */
  function postFile(endpoint: string, files: Record<string, File>, config?: IRequestConfig) {
    const isFile = (f: any) => Object.prototype.toString.call(f) === '[object File]';
    const fd = new FormData();
    Object.keys(files).forEach((k: string) => {
      if (!isFile(files[k])) throw new Error('postFile 方法只接受值为 File 对象的键值对作为参数！');
      fd.append(k, files[k]);
    });
   ...
   * @param {IRequestConfig} config
   */
  function patchJSON<T = any>(endpoint: string, data: Record<string, string | number> = {}, config?: IRequestConfig) {
   ...
   * @param {IRequestConfig} config
   */
  function deleteJSON<T = any>(endpoint: string, data: Record<string, string | number> = {}, config?: IRequestConfig) {
  ...
  function jsonp<T = any>(
    ...
    return new Promise((resolve, reject) => {
      ...
      JSONP(url, { prefix: `__${host.replace(/[^\w\d]/g, '')}` }, (err: null | Error, resp: any) => {
